...
This is not a safe default change yet. Applications can have response-writing middleware after the controller action, and a prematurely declared length could conflict with bytes appended later. Callers that own the complete pre-serialized body can set `response.content_length = body.bytesize` explicitly; before making that automatic, characterize after-handler and streaming behavior and measure on isolated Linux. Large dynamic JSON remains mostly serialization-bound even when the body is sent in one write.
